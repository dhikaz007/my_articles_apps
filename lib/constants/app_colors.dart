part of 'constants.dart';

class AppColors {
  static Color primaryGreen = const Color(0XFF247881);
  static Color primaryWhite = const Color(0XFFFFFFFF);
  static Color primaryBlack = const Color(0XFF000000);
  static Color primaryRed = const Color(0XFFB22727);
  static Color primaryGrey = const Color(0xff989898);
  static Color secondayBlue = const Color(0xff21BDC6);
  static Gradient primaryShimmerColor = const LinearGradient(
    colors: [
      Color(0xffC7C7C7),
      Color(0xffF9F8F8),
      Color(0xffC7C7C7),
    ],
  );
}
